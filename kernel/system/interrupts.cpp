...
InterruptManager* InterruptManager::singleton = 0;

void InterruptManager::SetInterruptDescriptorTableEntry(unsigned char interrupt,
    unsigned short CodeSegment, void (*handler)(), unsigned char DescriptorPrivilegeLevel, unsigned char DescriptorType)
{
...
InterruptManager::InterruptManager(unsigned short hardwareInterruptOffset, GlobalDescriptorTable* globalDescriptorTable/*, TaskManager* taskManager*/)
      ...
      programmableInterruptControllerSlaveDataPort(0xA1)
{
    ...
    unsigned int CodeSegment = globalDescriptorTable->CodeSegmentSelector();
...
unsigned short InterruptManager::HardwareInterruptOffset()
{
...
{
    if(singleton != 0)
        return singleton->DoHandleInterrupt(interrupt, esp);
    return esp;
}
...
unsigned int InterruptManager::DoHandleInterrupt(unsigned char interrupt, unsigned int esp)
{
    ...
    {
        //esp = (unsigned int)taskManager->Schedule((CPUState*)esp);
    }
