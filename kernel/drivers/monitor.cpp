...
Monitor::Monitor()
{
    ...
    this->text = (unsigned short *)0xB8000;
    this->color = 0x0F;
    this->x = this->y = 0;
    this->clear();
}
...
void Monitor::scroll()
{
    ...
    *  backcolor too */
    ...
    if(this->y >= 25)
    {
        /* Move the current text chunk that makes up the Monitor
        *  back in the buffer by a line */
        temp = this->y - 25 + 1;
        memcpy (this->text, this->text + temp * 80, (25 - temp) * 80 * 2);

        /* Finally, we set the chunk of memory that occupies
        *  the last line of text to our 'blank' character */
        memsetw (this->text + (25 - temp) * 80, blank, 80);
        this->y = 25 - 1;
    }
}
...
void Monitor::clear()
{
    ...
    *  color */
    for(i = 0; i < 25; i++)
        memsetw (this->text + i * 80, blank, 80);
...
void Monitor::putch(unsigned char c)
{
    ...
    *  Index = [(y * width) + x] */
    ...
    {
        where = this->text + (this->y * 80 + this->x);
...
void Monitor::puts(const char *str)
{
    int i;
